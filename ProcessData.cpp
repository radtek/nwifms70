...
int CProcessData::LoadTickets()
{
	...
	Sql = "SELECT A.TICKET_NUM, A.TICKET_NUM, A.COUNTERPARTY, A.ASSET_CODE, A.TRADER_INI, A.TRADE_DATE, A.EXEC_TIME, "
			...
			"NULL \"D_RECVD\", WI, NULL \"CONFIRM\", A.SWAP_TICKET, A.DELIVERY_DATE, A.MARGIN, A.MARGIN_CURRENCY, A"
			".MARGIN_AMOUNT, A.FUNDED_SWAP, A.BINARY, AA_METHOD, A.AA_FREASON, A.BEST_EXECUTION, A.SHORT_SALE, "
			...
			"A.ASSET_BUCKET, A.SOURCE, A.ASSET_COUPON, A.ASSET_MATURITY, B.ASS_PAR_VALUE, A.IMG_NOMINAL, A.CP_TRADE_ID, A.PRICE "
			...
			"A.ASSET_COUPON, A.ASSET_MATURITY, B.ASS_PAR_VALUE, A.IMG_NOMINAL, A.CP_TRADE_ID, A.PRICE "
			"FROM SEMAM.NW_RAW_TICKETS A, SEMAM.NW_ASSETS B "
			"WHERE B.ASS_CODE(+) = A.ASSET_CODE "
			"AND SIGN IS NOT NULL "
