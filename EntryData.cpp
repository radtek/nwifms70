...
int CEntryData::LoadTickets()
{
	...
	if(GetFXEntry())
	{
		Sql = "SELECT A.TICKET_NUM, A.TRADER_INI, TRADE_DATE, EXEC_TIME, TRANS_TYPE, DIR, VALUE_DATE, NOM_AMOUNT, "
				...
				"CSPB_SHORT, ETRADE, PRICE, DOWN_PYMNT, BROKER_FEE, OTHER_FEE, SOFTDOLLAR, CUSTODIAN_R, ASSIGN_CP, "
				"DV01, VAR, UNWIND_TICKET, CANCEL_TICKET, CORRECT_TICKET, ASSET_COUNTRY, ASSET_COMMON_CODE, "
				"ASSET_MSTC_CODE, ASSET_ISIN_CODE, ASSET_SEDOL_NUM, ASSET_CLASS, ASSET_IND_AREA, ASSET_CATEGORY, "
		...
		Sql +=	"AND (A.ASSET_CLASS IN ('CURRENCY FWDS', 'CURRENCY OPTION') OR TRANS_TYPE = 'FOREX') ";
				...
				"TO_NUMBER(NULL) \"OTHER_FEE\", TO_NUMBER(NULL) \"SOFTDOLLAR\", NULL \"CUSTODIAN_R\", NULL \"ASSIGN_CP\", "
	...
	else
	{
		if(GetBlotter())
			Sql = "SELECT TICKET_NUM, A.TRADER_INI, TRADE_DATE, EXEC_TIME, TRANS_TYPE, DIR, VALUE_DATE, NOM_AMOUNT, "
					...
					"ETRADE, PRICE, DOWN_PYMNT, BROKER_FEE, OTHER_FEE, SOFTDOLLAR, CUSTODIAN_R, ASSIGN_CP, DV01, VAR, "
					"UNWIND_TICKET, CANCEL_TICKET, CORRECT_TICKET, ASSET_COUNTRY, ASSET_COMMON_CODE, ASSET_MSTC_CODE, "
					"ASSET_ISIN_CODE, ASSET_SEDOL_NUM, ASSET_CLASS, ASSET_IND_AREA, ASSET_CATEGORY, ASSET_BUCKET, SOURCE, "
					"ASSET_COUPON, ASSET_MATURITY, REV, ORDER_ID "
		...
		else
		{
			Sql = "SELECT A.TICKET_NUM, A.TRADER_INI, TRADE_DATE, EXEC_TIME, TRANS_TYPE, DIR, VALUE_DATE, NOM_AMOUNT, "
					...
					"BROKER_FEE, OTHER_FEE, SOFTDOLLAR, CUSTODIAN_R, ASSIGN_CP, DV01, VAR, UNWIND_TICKET, CANCEL_TICKET, "
					"CORRECT_TICKET, ASSET_COUNTRY, ASSET_COMMON_CODE, ASSET_MSTC_CODE, ASSET_ISIN_CODE, ASSET_SEDOL_NUM, "
					"ASSET_CLASS, ASSET_IND_AREA, ASSET_CATEGORY, ASSET_BUCKET, A.SOURCE, ASSET_COUPON, ASSET_MATURITY, "
					"REV, ORDER_ID ";		
			if(GetBlock())
			...
			Sql += "UNION "
					...
					"TO_NUMBER(NULL) \"OTHER_FEE\", TO_NUMBER(NULL) \"SOFTDOLLAR\", NULL \"CUSTODIAN_R\", "
					...
					"B.ASS_ISIN_CODE, B.ASS_SEDOL_NUM, B.ASS_INDUSTRY, B.ASS_IND_AREA, B.ASS_CATEGORY, B.BUCKET, "
					"B.SOURCE, NULL, B.ASS_MATURITY_DATE, REV, A.ORDER_ID "
...
int CEntryData::LoadAllocList()
{
	...
	if(GetBlotter())
		GetOraLoader().GetSql().Format("SELECT A.TICKET_NUM, ROWIDTOCHAR(A.ROWID) \"ID\", A.PORTFOLIO, "
										"A.NOM_AMOUNT, A.ACCOUNT, A.PRICE, A.OPT_BACK "
	...
	if(GetOraLoader().Open())
	{
		...
		GetOraLoader().GetFieldArray().GetAt(0)->SetWithComma(FALSE);
		GetOraLoader().GetFieldArray().GetAt(6)->SetWithComma(FALSE);
