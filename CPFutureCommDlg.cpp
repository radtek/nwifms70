...
void CCPFutureCommDlg::RowToControls()
{
	if(m_SS.GetSheetCurRow() > 0 && m_SS.GetSheetCurRow() <= m_SS.GetSheetRows())
	{
		m_CP->SelectString(0, m_SS.GetSheetText(1, m_SS.GetSheetCurRow()));
		m_TransType->SelectString(0, m_SS.GetSheetText(2, m_SS.GetSheetCurRow()));
		m_Currency->SelectString(0, m_SS.GetSheetText(3, m_SS.GetSheetCurRow()));
		m_FutureType->SelectString(0, m_SS.GetSheetText(4, m_SS.GetSheetCurRow()));
		m_ETrade->SetCheck(m_SS.GetSheetText(5, m_SS.GetSheetCurRow()));
		m_Commission.SetWindowText(m_SS.GetSheetText(6, m_SS.GetSheetCurRow()));
		m_RowID = m_SS.GetSheetText(7, m_SS.GetSheetCurRow());
	}
}
...
void CCPFutureCommDlg::OnBnClickedFcLoadButton()
{
	...
	m_OraLoader.Open("SELECT COUNTERPARTY, TRANS_TYPE, CURRENCY, FUTURE_TYPE, ETRADE, "
						"COMMISSION, ROWIDTOCHAR(ROWID) \"ROWID\" "
...
void CCPFutureCommDlg::OnBnClickedFcAddButton()
{
	...
	m_OraLoader.Open("SELECT COUNTERPARTY, TRANS_TYPE, CURRENCY, FUTURE_TYPE, ETRADE, COMMISSION "
