...
BOOL CPayTemplate::LoadDBData()
{
	...
	GetOraLoader().GetSql() = "SELECT REF_ID, COUNTERPARTY, CURRENCY, BANK, ACCOUNT, CP_REF, "
								"BENEFICIARY, BENEFICIARY_ID, BENEFICIARY_BIC, BENEFICIARY_ACCOUNT, "
								"FFC, FFC_ID, FFC_BIC, FFC_ACCOUNT, ACTIVE "
								"FROM SEMAM.NW_PAY_TEMPLATE A, SEMAM.NW_COUNTER_PARTY B "
								"WHERE B.CP_CODE = A.COUNTERPARTY AND B.ISVALID = 'Y' ORDER BY 1 ";
...
int CPaymentData::LoadTemplates(const char *Portfolio, BOOL bLoad)
{
	...
	if(bRestricted)
		GetOraLoader().GetSql().Format("SELECT A.REF_ID, COUNTERPARTY, CURRENCY, A.BANK, "
							"BANK_ID_CODE, BANK_BIC, ACCOUNT, BENEFICIARY, BENEFICIARY_ID, "
							"BENEFICIARY_BIC, BENEFICIARY_ACCOUNT, FFC, FFC_ID, FFC_BIC, "
							"FFC_ACCOUNT, CP_REF "
							"FROM SEMAM.NW_PAY_TEMPLATE A, SEMAM.NW_BANKS B, SEMAM.NW_PAY_RESTRICTION C "
							"WHERE B.BANK = A.BANK "
		...
		GetOraLoader().GetSql() = "SELECT REF_ID, COUNTERPARTY, CURRENCY, A.BANK, BANK_ID_CODE, "
							...
							"FROM SEMAM.NW_PAY_TEMPLATE A, SEMAM.NW_BANKS B "
							"WHERE B.BANK = A.BANK "
							"AND A.ACTIVE = 'Y' "
							"ORDER BY 2 ";
	if(GetOraLoader().Open())
