	...
	if(GetBlotter())
	{
		if(IsTrader())
			Sql = "SELECT A.SIGN, A.TRADER_INI, A.TICKET_NUM, A.TRADE_DATE, A.EXEC_TIME, "
					...
					"A.ASSET_PID, A.EURO_OPT, A.OPT_AUTO, A.CURRENCY, A.FXRATE, A.COUNTERPARTY, A.ASSET_CODE, "
					"A.ASSET_DESC, A.ASSET_CURRENCY, A.ASSET_COUNTRY, A.ASSET_COMMON_CODE, A.ASSET_MSTC_CODE, "
					"A.ASSET_ISIN_CODE, A.ASSET_SEDOL_NUM, A.ASSET_CLASS, A.ASSET_IND_AREA, A.ASSET_CATEGORY, "
					"B.BUCKET_DESC, A.ASSET_COUPON, A.ASSET_MATURITY, A.IPO, A.TRS, A.FUNDED_SWAP, A.BINARY, "
					"A.REPO_CP, A.TR_RATE, A.RATE_BASIS, A.MATURITY, A.PRICE, A.ASSIGN_CP, A.NOTE, A.NOTE2, "
					"A.AA_METHOD, A.AA_FREASON, A.SW_BOOKING, A.SW_MATURITY, A.FLOAT_RATE, A.FLOAT_RATE_BASIS, "
					"A.FLOAT_MATURITY, A.WI, A.BOOKER, A.BEST_EXECUTION, A.SHORT_SALE, A.ETRADE, A.ORDER_ID, "
					"A.CP_TRADE_ID, A.ASSET_CLASS, REV, "
					"TO_CHAR(TRUNC(EMCF_B_WT, 0))||':'||TO_CHAR(TRUNC(PERFISO_WT, 0)) \"ALLOC\" "
					"FROM SEMAM.NW_RAW_TICKETS A, SEMAM.NW_BUCKETS B, SEMAM.NW_TRADERS C, SEMAM.NW_ASSETS D, "
					"SEMAM.NW_EMCFPERFISO_EQ_ALLOC_V E, SEMAM.NW_CURRENCY G "
					...
					"AND G.CURRENCY(+) = D.ASS_CURRENCY2 "
					...
					"TO_CHAR(TRUNC(EMCF_B_WT, 0))||':'||TO_CHAR(TRUNC(PERFISO_WT, 0)) \"ALLOC\" "
					"FROM SEMAM.NW_RAW_TICKETS A, SEMAM.NW_BUCKETS B, SEMAM.NW_TRADERS C, SEMAM.NW_ASSETS D, "
					"SEMAM.NW_EMCFPERFISO_EQ_ALLOC_V E, SEMAM.NW_INV_STRATEGIES F, SEMAM.NW_CURRENCY G "
					"WHERE B.BUCKET(+) = A.ASSET_BUCKET " 
					"AND D.ASS_CODE(+) = A.ASSET_CODE "
					"AND E.ASSET_CODE(+) = A.ASSET_CODE "
			...
			Sql = "SELECT A.SIGN, A.TRADER_INI, TO_CHAR(A.TICKET_NUM) \"TICKET_NUM\", A.TRADE_DATE, A.EXEC_TIME, "
					...
					"FROM SEMAM.NW_RAW_TICKETS A, SEMAM.NW_BUCKETS B, SEMAM.NW_ASSETS D, "
					"SEMAM.NW_EMCFPERFISO_EQ_ALLOC_V E, SEMAM.NW_CURRENCY G "
					"WHERE B.BUCKET(+) = A.ASSET_BUCKET "
					"AND D.ASS_CODE(+) = A.ASSET_CODE "
					"AND E.ASSET_CODE(+) = A.ASSET_CODE "
					...
					"NULL \"CP_TRADE_ID\", ASS_INDUSTRY, REV, "
					"TO_CHAR(TRUNC(EMCF_B_WT, 0))||':'||TO_CHAR(TRUNC(PERFISO_WT, 0)) \"ALLOC\" "
					"FROM SEMAM.NW_MATURITY_TRADE A, SEMAM.NW_ASSETS B, SEMAM.NW_BUCKETS C, "
					"SEMAM.NW_EMCFPERFISO_EQ_ALLOC_V E, SEMAM.NW_CURRENCY G "
					"WHERE B.ASS_CODE = A.ASSET_CODE "
					"AND C.BUCKET = B.BUCKET "
					"AND E.ASSET_CODE(+) = A.ASSET_CODE "
					"AND G.CURRENCY(+) = B.ASS_CURRENCY2 "
		...
		if(GetSign())
			...
			Sql = "SELECT A.SIGN, A.TRADER_INI, TO_CHAR(A.TICKET_NUM) \"TICKET_NUM\", A.TRADE_DATE, A.EXEC_TIME, "
					...
					"AND E.ASSET_CODE(+) = A.ASSET_CODE "
					"AND G.CURRENCY(+) = D.ASS_CURRENCY2 "
					"AND SIGN IS NULL AND (SYSDATE - A.DATE_STAMP)*24*60 > 2 "
				    "UNION "
					"SELECT A.SIGN, A.TRADER_INI, A.TICKET_NUM||'-'||TO_CHAR(A.INV_NUM), A.TRADE_DATE, NULL \"EXEC_TIME\", "
					"TO_CHAR(A.BOOK_DATE, 'MM/DD/YY HH24:MI:SS') \"DATE_STAMP\", A.VALUE_DATE, A.DIR, A.TRANS_TYPE, "
					"A.NOM_AMOUNT, TO_NUMBER(NULL) \"EXERCISE_PRICE\", TO_DATE(NULL) \"OPT_EXPIRATION\", NULL \"LISTED\", "
					...
					"B.ASS_COUNTRY, B.ASS_COMMON_CODE, B.ASS_MSTC_CODE, B.ASS_ISIN_CODE, B.ASS_SEDOL_NUM, B.ASS_INDUSTRY, "
					"B.ASS_IND_AREA, B.ASS_CATEGORY, C.BUCKET_DESC, NULL, B.ASS_MATURITY_DATE, NULL \"NEW_ISSUE\", "
					"A.DEAL_TYPE, NULL \"FUNDED_SWAP\", NULL \"BINARY\", NULL, TO_CHAR(A.RATE), A.RATE_BASIS, "
					"A.MATURITY, A.PRICE, A.ASSIGN_CP, NOTE, NOTE2, NULL, 'UNWIND LEGACY ASSET' \"AA_FREASON\", "
					"TO_DATE(NULL) \"ORIG_SW_BOOKING\", TO_DATE(NULL) \"ORIG_SW_MATURITY\", TO_NUMBER(NULL), NULL, "
					"TO_DATE(NULL), NULL \"WI\", A.BOOKER, 'OTHERS' \"BEST_EXECUTION\", NULL \"SHORT_SALE\", NULL, "
					"TO_NUMBER(NULL) \"ORDER_ID\", NULL \"CP_TRADE_ID\", ASS_INDUSTRY, REV, "
					"TO_CHAR(TRUNC(E.EMCF_B_WT, 0))||':'||TO_CHAR(TRUNC(E.PERFISO_WT, 0)) \"ALLOC\" "
					"FROM SEMAM.NW_MATURITY_TRADE A, SEMAM.NW_ASSETS B, SEMAM.NW_BUCKETS C, "
					"SEMAM.NW_EMCFPERFISO_OTHER_ALLOC_V E, SEMAM.NW_CURRENCY G "
					"WHERE B.ASS_CODE = A.ASSET_CODE "
					"AND C.BUCKET(+) = B.BUCKET "
...
BOOL CTicketApprovalData::ApprovalTicket(const CString SNote)
{
	...
	if(GetTicket().SignTicket(GetOraLoader(), GetSign()))
	{
		...
		if(!SNote.IsEmpty())
		{
			...
			Rec.Add(SNote);
			GetOraLoader().Open("SELECT COUNT(*) FROM SEMAM.NW_RAW_TICKET_OPINION WHERE TICKET_NUM = " + GetTicket().GetTicket());
			...
			{
				GetOraLoader().Open("SELECT TICKET_NUM, NOTE FROM SEMAM.NW_RAW_TICKET_OPINION WHERE TICKET_NUM = " + GetTicket().GetTicket(), ODYNASET_DEFAULT);
...
BOOL CTicketApprovalData::StraightThoughTrade()
{
	...
	if(OraLoader.GetCount("SELECT COUNT(*) FROM SEMAM.NW_TR_TICKETS WHERE TICKET_NUM = " + TicketNum) > 0)
	{
		OraLoader.GetSql() = "SELECT A.PORTFOLIO, C.TRANS_NUM, C.INV_NUM, NVL(A.CUSTODIAN_R, DEF_CUSTODIAN) \"CUSTODIAN\", "
	...
	{
		OraLoader.GetSql() = "SELECT A.PORTFOLIO, 0 \"TRANS_NUM\", 0 \"INV_NUM\", NVL(A.CUSTODIAN_R, DEF_CUSTODIAN) \"CUSTODIAN\", "
								"NVL(A.ACCOUNT, DEF_ACCOUNT) \"ACCOUNT\", A.NOM_AMOUNT, TRUNC(A.BROKER_FEE*A.NOM_AMOUNT/A.TOTAL_NOM_AMOUNT, 2) \"BR_FEES\", "
								"TRUNC(A.OTHER_FEE*A.NOM_AMOUNT/A.TOTAL_NOM_AMOUNT, 2) \"OTHER_FEES\", A.MARGIN, A.MARGIN_CURRENCY, "
								"TRUNC(A.MARGIN_AMOUNT*A.NOM_AMOUNT/A.TOTAL_NOM_AMOUNT, 2) \"MARGIN_AMOUNT\", "
								"TRUNC(A.VAR*A.NOM_AMOUNT/A.TOTAL_NOM_AMOUNT, 2) \"VAR\", A.CSPB_SHORT, A.PRICE, A.DOWN_PYMNT "
