...
int CFloatRepoData::LoadTickets(int nOrder)
{
	...
	GetOraLoader().GetSql().Format("SELECT PORTFOLIO, ASS_DESC, TRANS_NUM, ASSET_CODE, NOM_AMOUNT, "
							...
							"VALUE_DATE, MATURITY_DATE, NVL(START_DATE, ASS_FROM), ASS_TO, "
							"ASS_RATE_BASIS, ASS_TYPE, TRANS_TYPE, C.RATE, NVL(AMORT_FACT, 1) \"AMORT_FACT\", "
							"NVL(CAPIT_RATE, 0) \"CAP_RATE\", NVL(PLUS_AMOUNT, 0) \"PLUS_AMOUNT\", "
							"ASS_TO - ADD_MONTHS(ASS_TO, ROUND(MONTHS_BETWEEN(NVL(ASS_FROM), ASS_TO), 0)) \"AADAYS\", "
							...
							"AND FLOAT_RATE_FORMULA LIKE 'DAILY FLOAT%%' "
							"AND COUNTERPARTY = %s "
							"AND (MATURITY_DATE IS NULL OR MATURITY_DATE > TRUNC(SYSDATE)) ", (LPCTSTR) Text);
	if(nOrder > 0)
