...
int CAssetFindData::LoadDBData(BOOL bInitial)
{
	...
	NewSql = "SELECT A.ASSET_CODE, A.ASSET_DESC, A.ASSET_DESC2, A.COUNTRY, A.CURRENCY, A.TRANS_TYPE, "
				...
				"A.OPT_LISTED, REV, A.IDX "
				"FROM SEMAM.ALL_ASSETS_V A, SEMAM.NW_CURRENCY B "
				"WHERE B.CURRENCY(+) = A.CURRENCY2 "
				"AND A.ASSET_CODE = 'NEW ASSET' ";
	...
	else
	{
		...
		if(GetTicketBooking())
			Sql += " AND NVL(MATURITY_DATE, SYSDATE) > TRUNC(SYSDATE) - 14 ";
